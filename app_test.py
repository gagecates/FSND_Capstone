...
class MacroAppTestCase(TestCase):
    ...
    '''
        Tested theses next four by using the username 'Kevin'
        instead of a session key username
    '''
    ...
    def test_edit_food_fail(self):
        res = self.client().post(
            '/food/1000/edit', headers=get_headers(self.admin))
        data = json.loads(res.data)
    ...
    def test_delete_food_fail(self):
        res = self.client().post(
            '/food/1000/delete', headers=get_headers(self.admin))
        data = json.loads(res.data)
